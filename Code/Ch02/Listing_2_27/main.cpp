...
void wmain() {
	...
	sequence = ( ( BaseType* ) HeapAlloc( GetProcessHeap(), HEAP_ZERO_MEMORY, BufferSize ) );
	HeapFree( GetProcessHeap(), {}, reinterpret_cast< LPVOID >( SecureZeroMemory( reinterpret_cast< PVOID >( sequence ), BufferSize ) ) );
	sequence = nullptr;
	...
	wprintf_s( L"Value: %S\nLength: %u\nEmpty: %s", message->c_str(), message->length(), ( ( message->empty() ) ? L"Yes" : L"No" ) );
	message->clear();
