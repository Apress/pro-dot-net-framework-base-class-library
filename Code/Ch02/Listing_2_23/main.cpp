...
#pragma region Person custom reference type
...
void wmain() {
	...
	constexpr Int32 dimensions { 2 };

	/*

	Avoid the use of auto keyword with the C++/CLI, because the compiler does not have support for many scenarios and can generate error messages that can lead to waste of time.

	*/

	/*
	auto  objects { gcnew array<Object^, dimensions>{
		{ gcnew Person( "A" ) },
		{ gcnew String( "C++/CLI" ) },
		{ 10ui32 }
	} };
	*/
	...
	::cli::array<Object^, dimensions> ^ objects = gcnew ::cli::array<Object^, dimensions>{
		{ gcnew Person( "A" ) },
		{ gcnew String( "C++/CLI" ) },
		{ 10ui32 }
	};
	...
	Console::WriteLine( "objects == moreObjects: {0}", ( objects == moreObjects ).ToString() );
